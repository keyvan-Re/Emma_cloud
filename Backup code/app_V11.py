...
/* مخفی کردن آیکون/مربع نارنجی پیش‌فرض Gradio */
.progress-level svg, .progress-level img {
    display: none !important;
}

/* ایجاد یک دایره چرخان (Spinner) استاندارد */
.progress-level::before {
    content: "";
    display: inline-block;
    width: 20px;
    height: 20px;
    border: 3px solid #e5e7eb; /* رنگ پس‌زمینه دایره */
    border-radius: 50%;
    border-top-color: #0066cc; /* رنگ قسمت چرخنده دایره (آبی) */
...
"""
    ...
    # بقیه کدهای شما...
        ...
        def switch_user(state):
            ...
            return (
                gr.update(open=True),                   # 1. باز شدن سایدبار (login_page)
                gr.update(visible=False),               # 2. بسته شدن فرم ثبت‌نام (registration_fields)
        ...
        switch_user_btn.click(
            switch_user,
            inputs=[state],
            # اضافه کردن فیلدهای ثبت‌نام به خروجی‌ها
            outputs=[login_page, registration_fields, chat_page, login_status, username_input, state, chatbot, age_input, gender_input, occupation_input, residence_input],
            show_progress="hidden"
        )

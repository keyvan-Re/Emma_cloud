...
def create_gradio_interface(service_context, api_keys):
    with gr.Blocks(title="EMMA") as demo:
        ...
        def handle_register(name, age, gender, occupation, residence, state):
            ...
            welcome_msg = hello_msg if hello_msg else f"Welcome {name}! Registration complete."

            return (
                gr.update(elem_classes=["center-content", "slide-up"]), # 3. افزودن کلاس انیمیشن
                gr.update(visible=False),
                gr.update(visible=True), 
                gr.update(visible=False, value=""),
                gr.update(value=f"<h2 style='text-align: center; color: #333;'>🧠 EMMA: Session for {name}</h2>"),
    ...
    # اگر state خالی است یا کلید user_data را ندارد
            if not state or "user_data" not in state:
                print("Error: user_data not found in state.")
                # مقادیر بازگشتی را بر اساس خروجی‌های تابع خود تنظیم کنید
                # مثلاً برگرداندن state فعلی و یک لیست خالی برای چت‌باکس
                return state, [] 

            # دریافت امن اطلاعات کاربر
            user_data = state["user_data"]
            
            # --- بخش اول: استخراج حافظه از جلسه قبلی (قبل از ساخت جلسه جدید) ---
            if user_data.get("sessions") and len(user_data["sessions"]) > 0:
                last_session = user_data["sessions"][-1]
                
                # فقط در صورتی حافظه را استخراج کن که جلسه قبلی خالی نباشد (کاربر حرفی زده باشد)
                if len(last_session.get("conversation", [])) > 0:
                    
                    # استخراج حافظه اپیزودیک از مکالمات جلسه قبل
                    ep_summary = extract_session_summary(last_session["conversation"])
                    
                    # اضافه کردن تاریخ و ID همان جلسه به حافظه اپیزودیک استخراج شده
                    ep_summary["session_date"] = last_session["date"]
                    ep_summary["session_id"] = last_session.get("session_id", 0) 
                    
                    # ذخیره در لیست حافظه اپیزودیک
                    if "episodic_memory" not in user_data:
                        user_data["episodic_memory"] = []
                    user_data["episodic_memory"].append(ep_summary)
                    
                    # (اگر حافظه سمانتیک هم دارید، اینجا آپدیت کنید)

            # --- بخش دوم: ایجاد جلسه جدید با اختصاص ID ---
            
            # محاسبه ID جدید (بر اساس تعداد جلسات فعلی)
            new_session_id = len(user_data.get("sessions", []))
            
            new_session = {
                "session_id": new_session_id, # <--- مشکل اختصاص نیافتن ID اینجا حل می‌شود
                "date": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "conversation": []
            }
            ...
            # پاک کردن تاریخچه چت در رابط کاربری (Gradio)
            state["history"] = []

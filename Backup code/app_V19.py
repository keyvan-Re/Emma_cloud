    ...
    # بقیه کدهای شما...
        ...
        def handle_login_check(name, password, state):
            ...
            if name in memory:
                ...
                # اگر کاربر از قبل رمز دارد ولی رمز وارد شده اشتباه است
        ...
        check_user_btn.click(
            handle_login_check,
            inputs=[username_input, password_input, state], # password_input اضافه شد
            outputs=[login_page, registration_fields, chat_page, login_status, active_header, system_msg, state],
        )

        register_btn.click(
            handle_register,
            inputs=[username_input, password_input, age_input, gender_input, occupation_input, residence_input, state], # password_input اضافه شد
            outputs=[login_page, registration_fields, chat_page, login_status, active_header, system_msg, state],
        )
...
def main():
    ...
    if not gapgpt_api_key:
        ...
        # ست کردن متغیر محیطی برای جلوگیری از خطاهای ناگهانی کتابخانه‌های وابسته
        os.environ["OPENAI_API_KEY"] = gapgpt_api_key
        # حتماً Base URL را هم به متغیرهای محیطی اضافه کنید
    ...
    # تنظیم LLM (مدل زبانی) - این بخش برای چت است و درست کار می‌کند
    ...
    # تغییر بسیار مهم: مدل را به ada-002 تغییر دهید تا ثبت نام کاربر جدید کار کند
    embed_model = OpenAIEmbedding(
        api_key=gapgpt_api_key,
        api_base=GAPGPT_BASE_URL,
        model="text-embedding-ada-002" # <--- این خط تغییر کرد
    )

...
def create_gradio_interface(service_context, api_keys):
    with gr.Blocks(title="EMMA") as demo:
        ...
        # تابعی برای مدیریت کلیک دکمه ثبت نام
        def handle_registration():
            # کدهای مربوط به ذخیره کاربر در دیتابیس را اینجا قرار می‌دهید
            # سپس پنل ثبت‌نام را مخفی می‌کنیم و یک پیام به سیستم می‌فرستیم
            return gr.update(visible=False), "Registration successful!"

        with gr.Blocks() as demo:
            with gr.Row():
                
                # ==========================================
                # 1. LOGIN PAGE (ستون سمت چپ)
                # ==========================================
                with gr.Column(scale=1, min_width=300, elem_classes=["login-box"]) as login_page:
                    
                    # استفاده از Accordion برای ایجاد حالت کشویی
                    with gr.Accordion("🔐 Login & Registration Menu", open=True) as login_accordion:
            ...
            # 1. محو کردن بخش ثبت نام بعد از کلیک
            register_btn.click(
                fn=handle_registration,
                inputs=[], # فیلدهای ورودی مثل نام و سن را اینجا اضافه کنید
                outputs=[registration_fields, system_msg] # فرم ثبت‌نام را آپدیت (مخفی) می‌کند
            )
            
            # نکته: برای نمایش اولیه بخش ثبت نام، باید دکمه check_user_btn اینطور عمل کند:
            # check_user_btn.click(fn=check_user, inputs=username_input, outputs=registration_fields)
            # تا خروجی gr.update(visible=True) بدهد.
